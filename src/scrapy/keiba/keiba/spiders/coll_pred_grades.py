...
class CollEvalGradesSpider(scrapy.Spider):
  ...
  def parse(self, response):
    ...
    result_data = sel.css("div[class='RaceTableArea'] tr")
    # レースがない場合、処理を終了
    if not result_data:
      return
    for result in result_data:
      ...
      waku_num = result_td[0].css("span::text, ::text").get(default=0)
      horse_num = result_td[1].css("::text").get(default=0)
      ...
      sexual_age = result_td[4].css("::text").get(default='')
      load = result_td[5].css("::text").get(default=0)
      ...
      trainer = result_td[7].css("a::text").get(default='')
      trainer_id_info = result_td[7].css("a::attr(href)").get(default='')
      ...
      weight = result_td[8].css("::text").get(default='0')
      weight = re.sub(r'\n','',weight)
      weight_inc_dec = result_td[8].css("small::text").get(default='0')
